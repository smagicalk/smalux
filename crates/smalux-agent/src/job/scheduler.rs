...
use super::task::{ActionTask, AsyncCallback, TaskRegistration, ValueTask};
use super::*;
...
impl Scheduler {
    ...
    where
        T: ActionTask,
    {
        self.add(trigger, TaskRegistration::action(task), options)
            .await
    }
    ...
    where
        T: ValueTask,
    {
        self.add(trigger, TaskRegistration::channel(task, sender), options)
            .await
    }
    ...
    {
        self.add(trigger, TaskRegistration::callback(task, callback), options)
            .await
    }
