...
use super::super::defaults::{
    DEFAULT_PUBLIC_IP_LOOKUP_TIMEOUT, DEFAULT_PUBLIC_IP_MAX_CONCURRENCY,
    DEFAULT_PUBLIC_IP_REFRESH_INTERVAL, DEFAULT_PUBLIC_IP_RETRY_INTERVAL,
};
...
impl Default for PublicIpConfig {
    ...
    fn default() -> Self {
        Self {
            ...
            lookup_timeout: DEFAULT_PUBLIC_IP_LOOKUP_TIMEOUT,
...
impl PublicIpConfigPatch {
    ...
    pub(crate) fn apply_to(&self, config: &mut PublicIpConfig) {
        ...
        if let Some(lookup_timeout) = self.lookup_timeout {
            config.lookup_timeout = lookup_timeout;
        }
