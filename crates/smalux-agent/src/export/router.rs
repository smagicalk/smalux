...
use super::{ExportAdapter, ExportJobId, TransportHub, TransportPlan, TransportRequest};
...
impl ExportRouter {
    ...
    pub(crate) fn encode_report(
        ...
        self.adapter.encode_report(job_id, outbound)
    }
    ...
    pub(crate) async fn send_report(
        ...
        let requests = self.encode_report(job_id, outbound)?;
        let request_count = requests.len();

        // adapter 可以返回 0 条请求，表示当前格式不支持或明确跳过该事件。
        // router 只负责投递，不把“跳过”当错误。
        for request in requests {
            transport_hub.enqueue(job_id, outbound.sequence, request)?;
        }
    ...
    pub(crate) async fn send_remote_task_result(
        ...
        for request in requests {
            transport_hub.enqueue(ExportJobId::RemoteTaskResult, result.sequence, request)?;
        }
    ...
    pub(crate) async fn send_remote_probe_result(
        ...
        for request in requests {
            transport_hub.enqueue(ExportJobId::RemoteProbeResult, result.sequence, request)?;
        }
    ...
    pub(crate) async fn send_control_ack(
        ...
        for request in requests {
            transport_hub.enqueue(ExportJobId::ControlAck, ack.sequence, request)?;
        }
    ...
    pub(crate) async fn send_control_error(
        ...
        for request in requests {
            transport_hub.enqueue(ExportJobId::ControlError, error.sequence, request)?;
        }
