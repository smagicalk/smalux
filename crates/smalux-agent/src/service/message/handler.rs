...
//! adapter/handler 中先转换为统一内部命令，不能绕回这里解析 raw JSON。

use crate::config::ConfigManager;
use crate::config::model::AgentConfigPatch;
use crate::export::{InboundProtocolHandler, TransportInboundMessage, inbound_message_into_string};
...
/// Smalux 自有协议控制消息入站处理器，负责把 server 控制消息转换成内部命令。
#[derive(Debug, Clone)]
pub(crate) struct SmaluxControlHandler {
...
impl SmaluxControlHandler {
    ...
    /// 解析一条 server 控制消息。
    ///
    /// WebSocket transport 已经在进入 handler 前完成 wire 解包和可选解密，这里收到的
...
impl InboundProtocolHandler for SmaluxControlHandler {
...
mod tests {
    ...
    /// 构造测试 handler。
    fn handler(agent_id: &str) -> SmaluxControlHandler {
        ...
        SmaluxControlHandler::new(config_manager, commands)
    }
