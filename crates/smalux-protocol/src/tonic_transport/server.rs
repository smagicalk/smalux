...
use crate::{
    agent::v1::{
        NoiseHandshake, ProtocolFrame, SecureError, SecureMessage, noise_handshake::HandshakeType,
...
impl ServerSessionAcceptor {
    ...
    ///
    /// `sender` 与 `inbound` 必须属于同一个 RPC；`enrollment_psk` 只用于 XXpsk3。
    pub async fn accept_session(
        ...
        let established = match kind {
            HandshakeType::XxPsk3 => {
                ...
                let (waiting, second) =
                    ServerXxHandshake::receive_message1(identity, enrollment_psk, first)?;
