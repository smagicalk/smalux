...
完整上报 JSON 参数见 `crates/smalux-agent/README.md` 的 `ClientFrame` 和 `AgentReport` JSONC 示例。server 侧只接收实际标准 JSON，不接收文档里的注释。
...
server 后续通过同一条 Smalux WebSocket 控制通道下发 JSON。当前 agent 支持：

- `config_patch`：动态调整 `AgentConfig` 中的采集、上报和导出参数。
