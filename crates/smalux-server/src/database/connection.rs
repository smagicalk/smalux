...
impl DatabaseConfig {
    ...
    fn resolve_connection_url(&self) -> Result<(String, DatabaseBackend), DatabaseError> {
        let mut url =
            Url::parse(&self.url).map_err(|error| DatabaseError::InvalidUrl(error.to_string()))?;
...
/// Server 进程持有的数据库句柄。
///
/// DatabaseConnection 是可克隆的连接池句柄，因此后续 repository 可以按需克隆它；
...
mod tests {
    ...
    use super::{DatabaseConfig, ServerDatabase};
